...
exports.addMedia = asyncWrapper(async (req, res) => {
  ...
  if (!title || !["image", "video", "ebook", "audio"].includes(type)) {
    return res.status(StatusCodes.BAD_REQUEST).render("status/status", {
      ...
      keywords: "media, error",
      status: 400,
      message_title: "Invalid Media Info",
      message: "Title and valid media type are required.",
      actionUrl: "/media",
      actionText: "Go back to media",
    });
  }
  ...
  if (req.files?.image) {
    ...
    const base = slugify(path.basename(thumbnailFile.name, ext));
    const filename = `${Date.now()}-${base}${ext}`;
    const fullPath = path.join(thumbnailDir, filename);

    await thumbnailFile.mv(fullPath);
    image = `/asset/media/thumbnails/${filename}`;
  }

  // Handle media upload
  if (type !== "video" && req.files?.file) {
    ...
    const ext = path.extname(mediaFile.name);
    const base = slugify(path.basename(mediaFile.name, ext));
    const filename = `${Date.now()}-${base}${ext}`;
    const fullPath = path.join(uploadDir, filename);

    await mediaFile.mv(fullPath);
    url = `/asset/media/${type}/${filename}`;
  }
...
exports.updateMedia = asyncWrapper(async (req, res) => {
  ...
  if (req.files?.image) {
    ...
    if (media.image) {
      try {
        await fs.unlink(path.join("public", media.image));
      } catch (e) {
  ...
  if (type !== "video" && req.files?.file) {
    ...
    if (media.url) {
      try {
        await fs.unlink(path.join("public", media.url));
      } catch (e) {
        console.warn("Could not delete old media:", e.message);
      }
    }
  ...
  media.url = url;
  media.image = image;
  media.tags = tags ? tags.split(",").map((t) => t.trim()) : media.tags;
